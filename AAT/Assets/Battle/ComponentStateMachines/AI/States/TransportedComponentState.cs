...
public class TransportedComponentState : InteractionComponentState
{
    ...
    public override void Spawned()
    {
        ...
        _targetFinder = new TargetFinder(GetComponent<TeamController>(), _stats.GetModifiedStat(EUnitFloatStats.AttackRange), true);
        _moveSystem = Container.GetComponent<IMoveSystem>();
        _death = Container.GetComponent<DeathController>();
        _selectable = Container.GetComponent<Selectable>();
    ...
    public void Mount(BaseMountableController mount)
    {
        ...
        _mount.Selectable.OnDeath += MountDeathHandler;*/
    }
    
    private void MountDeathHandler()
    {
        _death.Die();
    }
