...
public class SectorProgressListener : MonoBehaviour
{
    ...
    private void HandleSectorCaptureChanged(PlayerRef player, int amount)
    {
        ...
        if (TargetHelper.TargetRelation(_cachedTeam, ownerTeam, ETargetRelation.None))
        {
            image.material.color = relationColors.Colors[0];
        }
        if (TargetHelper.TargetRelation(_cachedTeam, ownerTeam, ETargetRelation.Owned))
        {
            image.material.color = relationColors.Colors[1];
        }
        if (TargetHelper.TargetRelation(_cachedTeam, ownerTeam, ETargetRelation.Ally))
        {
            image.material.color = relationColors.Colors[2];
        }
        if (TargetHelper.TargetRelation(_cachedTeam, ownerTeam, ETargetRelation.Enemy))
        {
